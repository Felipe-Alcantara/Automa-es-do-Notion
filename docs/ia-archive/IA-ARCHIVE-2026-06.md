# 🗄️ IA-ARCHIVE — junho/2026 (era monorepo)

> Registros movidos **na íntegra e sem edição** do `IA.md` do hub em 2026-07-18,
> seguindo a estratégia de compactação sem perda do template de contexto do
> Felixo System Design. Nada foi apagado: a linha do tempo completa do projeto é
> `IA.md` + este arquivo, em ordem cronológica.
>
> Estes registros descrevem a **era monorepo** — antes da separação do código em
> módulos (`notion-starter`, `notion-tasks-cli`, `notion-workspace-app`). Os
> caminhos citados (`src/`, `cli/`, `server/`, `front/`) hoje vivem nos
> repositórios de cada módulo; veja a nota de estrutura no `IA.md` do hub.
